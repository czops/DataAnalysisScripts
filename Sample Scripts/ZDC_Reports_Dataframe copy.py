...
def find_InOut():
    ...
    #ProdData2 = ProdData2.drop(ProdData2['Process_Time'] > 8)

#correctingDates()
find_InOut()

ProdDataNoNegs = ProdData2.drop(ProdData2['Process_Time'] < 0)

#DF for plotting recipe times
...
#ProdDataNoNegsNoErrors = ProdDataNoNegs.drop(ProdDataNoNegs['Process_Time'] > 5)

def plotCleaningNegatives():
    ProdDataNoNegs = ProdData2.drop(ProdData2['Process_Time'] < 0)
    return ProdDataNoNegs

#plotCleaningNegatives()

def plotCleaningErrors():
    ProdDataNoNegsNoErrors = ProdDataNoNegs.drop(ProdDataNoNegs['Process_Time'] > 5)
    return ProdDataNoNegsNoErrors

#plotCleaningErrors()

ProdDataNoNegs.to_csv('cleanedData.csv')
